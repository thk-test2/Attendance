...
public:
    ...
    virtual int bonusPointForTrainingDay(UserInfo& userInfo) = 0;
    virtual int bonusPointForWeekends(UserInfo& userInfo) = 0;
    virtual std::string getUserGrade(int totalPoints) = 0;
    virtual void evaluateGrade(UserInfo& userInfo) = 0;
    virtual bool isRemovedPlayer(UserInfo& userInfo) = 0;
};
...
public:
    ...
    int bonusPointForTrainingDay(UserInfo& userInfo) override {
    ...
    int bonusPointForWeekends(UserInfo& userInfo) override {
    ...
    bool isRemovedPlayer(UserInfo& userInfo) override {
...
bool AttendanceSystem::isNewUser(std::string& userName, IdInfo& idInfo) {
    return idInfo.idTable.count(userName) == 0;
}

void AttendanceSystem::addNewUser(IdInfo& idInfo, std::string& userName,
    std::array<UserInfo, MAX_USER_CNT>& userInfoList) {
    idInfo.idTable.insert({ userName, ++(idInfo.totalUsers) });
    userInfoList.at(idInfo.totalUsers).name = userName;
}

int AttendanceSystem::getUniformId(std::string& userName, IdInfo& idInfo) {
    return idInfo.idTable[userName];
}

void AttendanceSystem::checkIfDebugData(std::string& userName) {
    if (userName == "Daisy") int debug = 1;
}

int AttendanceSystem::getDayIndex(std::string& dayOfWeek) {
...
void AttendanceSystem::printUserInfo(UserInfo& userInfo) {
    std::cout << "NAME : " << userInfo.name << ", ";
    std::cout << "POINT : " << userInfo.totalPoints << ", ";
    std::cout << "GRADE : " << userInfo.grade << "\n";
}

void AttendanceSystem::printRemovedUser(IdInfo& idInfo,
    std::array<UserInfo, MAX_USER_CNT>& userInfoList) {
    std::cout << "\n";
...
void AttendanceSystem::analyzeEachRecord(std::string& userName, std::string& dayOfWeek,
    IdInfo& idInfo,
    std::array<UserInfo, MAX_USER_CNT>& userInfoList) {
    ...
    if (policy->isSpecialDay(getDayIndex(dayOfWeek))) userInfo.specialDayAttended = true;

    userInfo.attendHistory[getDayIndex(dayOfWeek)] += 1;
    userInfo.totalPoints += policy->getDayPoints(getDayIndex(dayOfWeek));
}

void AttendanceSystem::analyzeAllRecord(IdInfo& idInfo,
    std::array<UserInfo, MAX_USER_CNT>& userInfoList) {
